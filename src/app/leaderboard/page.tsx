...
export default function LeaderboardPage() {
        ...
        return (
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
                ...
                <div className="text-center py-20 bg-surface-light rounded-none border-2 border-dashed border-surface-lighter">
                    <p className="text-ink/60 font-medium font-mono">Leaderboard isn't active yet.</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
            ...
            {/* Podium Section */}
            <div className="flex flex-col md:flex-row items-end justify-center gap-4 md:gap-8 mb-16 px-4 pt-8">
                {/* 2nd Place */}
                ...
                <div className="max-w-4xl mx-auto space-y-4">
                    ...
                    <div className="grid gap-2">
                        {currentUsers.map((user, i) => (
                            <div key={user.uid} className="flex items-center justify-between p-4 bg-surface-light border border-surface-lighter hover:border-primary transition-colors group rounded-none">
                                <div className="flex items-center gap-4">
                                    <span className="font-mono text-ink/75 w-8">#{startIndex + i + 4}</span>
                                ...
                                <span className="font-mono font-bold text-ink">{user.points || 0} pts</span>
                            </div>
                        ))}
                    </div>
                    ...
                    {totalPages > 1 && (
                        <div className="flex justify-center gap-2 mt-8">
                            ...
                            <span className="font-mono text-sm self-center px-4 text-ink/70">Page {currentPage} of {totalPages}</span>
...
function PodiumUser({ user, rank, color, bgColor, borderColor, height, isFirst = false }: PodiumUserProps) {
    return (
        <div className={`flex flex-col items-center w-full md:w-1/3 ${isFirst ? 'order-first md:order-0 z-10' : ''}`}>
            ...
            <div className={`w-full ${height} ${bgColor} border-t-4 border-l border-r ${borderColor} flex flex-col items-center justify-start py-4 relative group transition-all hover:brightness-105`}>
                ...
                <div className="font-mono text-ink/60 font-medium z-10">{user.points || 0} pts</div>
            </div>
        </div>
    )
}
